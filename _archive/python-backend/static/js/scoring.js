...
export function aggregateWeights(allParticipantAnswers) {
  // allParticipantAnswers: Array of Arrays of { question_id, choice_id } with weights resolved
  // Each entry: { weights: {dim: val, ...} }
  const totals = {};
  const budgetVotes = [];
...
export function rankRestaurants(restaurants, weights, recentNames, yesterdayNames, topN = 20, blacklist = []) {
  ...
  // Cuisine fatigue (requires recentCuisineCounts passed in weights as _cuisineCounts)
